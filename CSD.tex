...
We publish all code\footnote{\url{https://github.com/Jakob-Bach/Constrained-Subgroup-Discovery}} and experimental data\footnote{\url{https://doi.org/10.35097/caKKJCtoKqgxyvqG}} online.
...
The chosen feature-cardinality constraint (cf.~Definition~\ref{def:csd:feature-cardinality-constraint}) is \emph{antimonotonic}~\cite{ng1998exploratory} in the feature selection:
...
\end{proposition}
%
The evaluation of solution candidates differs from the original subgroup descriptions but has a similar complexity, i.e., $O(m \cdot n + a \cdot n)$ instead of $O(m \cdot n)$.
