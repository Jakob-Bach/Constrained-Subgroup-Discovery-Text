...
background knowledge \cite{atzmueller2005exploiting, atzmueller2006methodological, atzmueller2007using, atzmueller2009semi}
according to \cite{meeng2021real}, lower bounds on data objects and subgroup quality are common constraints in SD algorithms, as well as upper bound on search depth (feature cardinality)
\cite{lavravc2006relevancy} mentions feature-cardinality constraints, recall constraints
\cite{helal2016subgroup, herrera2011overview, ventura2018subgroup} mention number of features (and number of subgroups) as complexity measure
\cite{friedman1999bump} also mentions feature cardinality as complexity measure and lower complexity as desirable for interpretability
...
Related work also uses the terms \emph{depth} \cite{meeng2021real} or \emph{length} \cite{atzmueller2015subgroup, helal2016subgroup} for this quantity, though partly referring to the number of conditions in the subgroup description rather than selected features.
...
%
\begin{proposition}[Complexity of perfect-subgroup discovery with a feature-cardinality constraint]
...
We set the peeling fraction of PRIM to~$\alpha = 0.05$, consistent with other implementations of PRIM~\cite{arzamasov2021reds, kwakkel2017the} and within the recommended value range proposed by the original authors~\cite{friedman1999bump}.
...
Two assumptions for subgroup discovery in our paper were numerical features and a binary target (cf.~Section~\ref{sec:csd:fundamentals:problem}).
...
In particular, we define the objective as optimizing a subgroup-quality function~$Q(\mathit{lb}, \mathit{ub}, X, y)$ rather than searching for a perfect subgroup that may or may not exist.
...
We define the original subgroup to be perfect, i.e.,  subgroup membership $b^{(0)} = y$.
...
In particular, we define the features $n+1, \dots, n+k$ to be identical to the prediction target~$y$.
Choosing the bounds $\mathit{lb}_j = \mathit{ub}_j = 1$ on any of these extra features produces the defined subgroup membership $b^{(0)} = y$.
We further assume all extra features were selected in the original subgroup description but none of the actual features from~$X$, i.e., $\forall j \in \{1, \dots, n\}:~ s^{(0)}_j = 0$ and $\forall j \in \{n+1, \dots, n+k\}:~ s^{(0)}_j = 1$.

A solution for problem instance~$I'$ also is a solution for problem instance~$I$.
In particular, the perfect alternative subgroup description (cf.~Definition~\ref{def:csd:perfect-alternative}) is a perfect subgroup since it perfectly reproduces the original subgroup description, which is a perfect subgroup.
I.e., $b^{(a)} = b^{(0)} = y$.
...
Finally, both~$I$ and~$I'$ use a feature-cardinality constraint with threshold~$\tau$.
Thus, if $b^{(a)}$ and $s^{(a)}$ satisfying the constraints exists, they solve~$I$.
If they do not exist, then there also is no other perfect subgroup solving~$I$.
...
Further, evaluating a solution for the former problem entails a cost of $O(m \cdot n)$ for checking subgroup membership, the feature-cardinality constraint, and the dissimilarity constraint, placing the problem in complexity class~$\mathcal{NP}$.
