...
(Case 3) $\mathit{lb}_j = 0,~\mathit{ub}_j = 1$ explicitly deselects a binary feature, i.e., both binary values do not exclude subgroup membership.
...
Assume the categorical Feature~$j$ has $|c| \in \mathbb{N}$ different categories~$\{c_1, \dots, c_{|c|}\}$.
Let $\mathit{cb}_j \in \{0, 1\}^{|c|}$ denote the corresponding bound variables, which denote whether a category is included in the subgroup.
...
%
\begin{equation}
	\bigvee_{l \in \{1, \dots, |c|\}} \left( \mathit{cb}^l_j \land \left(  X_{ij} = c_l \right) \right)
\end{equation}
%
...
%
\begin{equation}
	s_j \leftrightarrow \lnot \bigwedge_{l \in \{1, \dots, |c|\}} \mathit{cb}^l_j
\end{equation}
%
...
%
\begin{equation}
	 \left( \left( \sum_{l=1}^{|c|} \mathit{cb}^l_j \right) = 1 \right) \lor \left( \left( \sum_{l=1}^{|c|} \mathit{cb}^l_j \right) = |c| \right)
	\label{eq:csd:constraint:category-cardinality}
\end{equation}
