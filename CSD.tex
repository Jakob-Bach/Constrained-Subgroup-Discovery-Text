...
Both methods require discretization of numeric features, which makes them non-exhaustive regarding the full numeric search space in our experiments.
...
While the former two are exhaustive, they require discretization of numeric features.
...
For most subgroup-discovery methods, subgroups with as few as $k=2$ feature in their description often yielded already a similar quality as unconstrained subgroups, i.e., using all features.
...
These result speak for using small feature sets in subgroup descriptions, which may also benefit interpretability for users.
...
The goal of the competitor-runtime experiments is to find subgroup-discovery methods that are suitable competitors for solver-based search in our main experimental pipeline.
...
\begin{table}[t]
	\centering
	\caption{
		Datasets from PMLB used in our competitor-runtime experiments.
		$m$~denotes the number of data objects and $n$~the number of features.
	}
	\begin{tabular}{lrr}
		\toprule
...
The methods from \emph{sd4py} and \emph{subgroups} require discretization of numeric features.
...
Both methods retain the potential weakness that they require discretization of numeric features, which reduces their search space.
