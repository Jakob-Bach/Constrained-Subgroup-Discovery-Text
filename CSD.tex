...
The `covering' approach allows sequential search for any subgroup-discovery method by removing all data objects covered by previous subgroups and repeating subgroup discovery~\cite{friedman1999bump}.
Alternatively, one may resample data objects based on their subgroup membership~\cite{scholz2005sampling}.
In contrast, simultaneous search requires subgroup-discovery methods that specifically target at multiple solutions~\cite{leeuwen2012diverse, lucas2018ssdp+, proencca2022robust}.
...
Both constraint types we analyze, i.e., feature-cardinality constraints and alternative subgroup descriptions, relate to the features used in the subgroup.
...
There are white-box formulations for various types of classification models.
...
Similar to subgroups, these three model types also use conjunctions of binary checks to form decision rules.
However, they use multiple rules to classify data globally, while subgroup discovery employs one rule to describe an interesting region.
...
Approaches yielding multiple counterfactual often foster diversity, e.g., by extending the optimization objective~\cite{mothilal2020explaining} or introducing constraints~\cite{karimi2020model, mohammadi2021scaling, russell2019efficient}.
...
Our notion of alternative subgroup descriptions also pursues a similarity objective based on data objects.
However, we search for alternative descriptions sequentially instead of simultaneously.
The original subgroup description usually optimizes subgroup quality, while redescription mining has no target variable, i.e., is unsupervised~\cite{ramakrishnan2004turning}.
...
In contrast, we provide the parameter~$\tau$ to allow users to control the overlap between feature sets. (cf.~Equation~\ref{eq:csd:smt-constraint-dissimilarity}.
...
Finally, existing approaches for redescription mining are algorithmic rather than using white-box optimization~\cite{galbrun2017redescription, mihelcic2023complexity}.
