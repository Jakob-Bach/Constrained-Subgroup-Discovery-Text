...
\begin{definition}[Subgroup discovery]
	...
	\emph{subgroup discovery} is the problem of finding a subgroup (cf.~Definition~\ref{def:csd:subgroup}) with bounds~$\mathit{lb}, \mathit{ub} \in \mathbb{R}^n$ that maximize a given notion of subgroup quality~$Q(\mathit{lb}, \mathit{ub}, X, y)$.
...
\begin{algorithm}[p]
	...
	\While(\tcp*[f]{Support threshold satisfied}){$\frac{m_b}{m} > \beta_0$}{ \label{al:csd:prim:line:stop} \label{al:csd:prim:iteration-start}
		...
		\For{$j \in$ get\_permissible\_feature\_idxs(\dots)}{ \label{al:csd:prim:line:peel-start}
...
cardinality also known as depth \cite{meeng2021real} or length \cite{atzmueller2015subgroup, helal2016subgroup}, though that may refer to number of conditions (and interval may count as two conditions)

complex subgroup descriptions bad for interpretability \cite{meeng2021real}

as alternative to prescribing k, can also eliminate irrelevant variables after having found subgroup \cite{friedman1999bump}
