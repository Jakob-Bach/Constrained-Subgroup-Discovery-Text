...
Slightly deviating from the previously described criterion, we set bounds to infinity if the minimum/maximum feature value of positive data objects equals the minimum/maximum feature value of all data objects (Lines~\ref{al:csd:morb:line:bounds-infty-start}-\ref{al:csd:morb:line:bounds-infty-end}).
...
It generates and evaluates subgroups for a fixed number of iterations, which the user conrols with the parameter~$\mathit{n\_iters}$.
...
As for \emph{MORB} (cf.~Algorithm~\ref{al:csd:morb}), we replace all non-excluding bounds with infinity (Lines~\ref{al:csd:random-search:line:bounds-infty-start}-\ref{al:csd:random-search:line:bounds-infty-end}).
