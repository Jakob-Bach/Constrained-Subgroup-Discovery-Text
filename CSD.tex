...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=20 60 15 15, clip]{plots/csd-methods-unconstrained-nwracc-all-datasets.pdf}
		\caption{All datasets.}
		\label{fig:csd:methods-unconstrained-nwracc-all-datasets}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth, trim=20 60 15 15, clip]{plots/csd-methods-unconstrained-nwracc-no-timeout-datasets.pdf}
		\caption{Datasets without \emph{SMT} timeouts.}
		\label{fig:csd:methods-unconstrained-nwracc-no-timeout-datasets}
	\end{subfigure}
	\caption{
	...
	\label{fig:csd:methods-unconstrained-nwracc}
\end{figure}

\paragraph{Subgroup quality}

Figure~\ref{fig:csd:methods-unconstrained-nwracc-all-datasets} compares subgroup quality on the training set and test set for the six subgroup-discovery methods.
...
However, even if we limit our analysis to the datasets without \emph{SMT} timeouts, \emph{Beam} and \emph{BI} are still remarkably close to the optimum quality (cf.~Figure~\ref{fig:csd:methods-unconstrained-nwracc-no-timeout-datasets}).
...
\begin{table}
	...
	\begin{subtable}{\textwidth}
		...
		\label{tab:csd:methods-unconstrained-runtime-all-datasets}
	\end{subtable}
	\\ \vspace{\baselineskip}
	\begin{subtable}{\textwidth}
		\centering
		...
		\label{tab:csd:methods-unconstrained-runtime-no-timeout-datasets}
	\end{subtable}
	\caption{
		Aggregated runtime over datasets and cross-validation folds, by subgroup-discovery method.
		Results from the unconstrained experimental scenario.
	}
	\label{tab:csd:methods-unconstrained-runtime}
\end{table}

\paragraph{Runtime}

Table~\ref{tab:csd:methods-unconstrained-runtime} summarizes the runtimes of the subgroup-discovery methods.
...
\begin{table}
	...
	\label{tab:csd:methods-unconstrained-runtime-correlation}
\end{table}
...
In particular, Table~\ref{tab:csd:methods-unconstrained-runtime-correlation} considers the number of data objects~$m$, the number of features~$n$, the product of these two quantities~$m \cdot n$, and the number of unique values per feature summed over the features~$\Sigma n^u$.
...
E.g., with a solver timeout of 64~s, corresponding to an average overall runtime of 89~s, \emph{SMT} achieves a training-set nWRAcc of 0.44, compared to 0.56 for \emph{Beam} with an average runtime of 33~s (cf.~Table~\ref{tab:csd:methods-unconstrained-runtime-all-datasets}).
