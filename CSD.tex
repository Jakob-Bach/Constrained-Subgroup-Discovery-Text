...
E.g., with a solver timeout of 64~s, corresponding to an average overall runtime of 89~s, \emph{SMT} achieves a mean training-set nWRAcc of 0.44, compared to 0.56 for \emph{Beam} with an average runtime of 33~s (cf.~Table~\ref{tab:csd:unconstrained-methods-runtime-all-datasets}).
...
For most subgroup-discovery methods, mean training-set nWRACC increases with~$k$, though the marginal utility decreases.
...
On the test set, the benefit of setting a larger~$k$ is even smaller.
...
\begin{table}
	...
	\label{tab:csd:cardinality-methods-runtime}
\end{table}

\paragraph{Runtime}

As Table~\ref{tab:csd:cardinality-methods-runtime} displays, the heuristic search methods \emph{Beam}, \emph{BI}, and \emph{PRIM} become faster the smaller~$k$ is.
