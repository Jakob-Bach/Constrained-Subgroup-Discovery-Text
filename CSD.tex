...
\end{proposition}
%
For the special case $k=1$, the size of the search space becomes $O(m \cdot n^2)$, which is leveraged by heuristic search methods that only consider updating the bounds of each feature separately instead of jointly (cf.~Section~\ref{sec:csd:fundamentals:heuristics}).
...
We implemented all subgroup-discovery methods, experiments, and evaluations in Python~3.8 and released the code online\footnote{\url{https://github.com/Jakob-Bach/Constrained-Subgroup-Discovery}}.
