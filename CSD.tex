...
To find optimal subgroups exactly, one can formulate subgroup discovery as a white-box optimization problem and employ a solver.
...
For our formulation of optimal subgroup discovery, we use linear real arithmetic (LRA).
Complementing this SMT formulation, Appendix~\ref{sec:csd:appendix:further-encodings} describes encodings with mixed-integer programming (MIP) and maximum satisfiability (MaxSAT), which require more decision variables.
...
\end{equation}
%
Instead of defining separate decision variables~$b_i$ and binding them to the bounds with a equivalence constraint, one could also insert the Boolean expression on the right-hand-side directly into Equation~\ref{eq:csd:constraint-m-as-sum}.
